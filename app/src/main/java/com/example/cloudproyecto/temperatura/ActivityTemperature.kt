...
class ActivityTemperature : AppCompatActivity(), ContractTemperature.View, OnChartValueSelectedListener {

    private lateinit var mTempChart: LineChart
    lateinit var mPresenter: ContractTemperature.Presenter
    ...
    companion object {
        fun newInstance(ctx: Context): Intent {
            val tempActivity = Intent(ctx, ActivityTemperature::class.java)
            return tempActivity
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mTempChart = findViewById(R.id.line_chart_temp)
        mTempChart.setOnChartValueSelectedListener(this)
        mPresenter = PresenterTemperature(this)
        // espera 100 mili segundos y repite cada 5 segundos (5000 milisegundos)
    ...
    override fun setData(dataListJson: JSONArray) {
        ...
        for (i in 0 until dataListJson.length()) {
            println("Temperature ==> ${(dataListJson[i] as JSONObject).get("temperature")}")
            addEntry((dataListJson[i] as JSONObject).get("temperature").toString().toFloat())
        }
    }
    ...
    private fun addEntry(xiData: Float) {
        ...
        if (data != null) {
            ...
            data.addEntry(
                Entry(set!!.entryCount.toFloat(), xiData),
                0
    ...
    private fun createSet(): ILineDataSet? {
        val set: ILineDataSet? = LineDataSet(null, "Temperature data")
        // change color
        (set as LineDataSet).color = Color.MAGENTA
