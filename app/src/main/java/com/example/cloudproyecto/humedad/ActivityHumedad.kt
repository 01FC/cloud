...
class ActivityHumedad : AppCompatActivity(), OnChartValueSelectedListener, ContractHumedad.View {

    private lateinit var mTempChart: LineChart
    lateinit var mPresenter: ContractHumedad.Presenter
    ...
    companion object {
        fun newInstance(ctx: Context): Intent {
            val tempActivity = Intent(ctx, ActivityHumedad::class.java)
            return tempActivity
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mTempChart = findViewById(R.id.line_chart_hum)
        mTempChart.setOnChartValueSelectedListener(this)
        mPresenter = PresenterHumedad(this)
    ...
    override fun setData(dataListJson: JSONArray) {
        ...
        for (i in 0 until dataListJson.length()) {
            println("Humedad ==> ${(dataListJson[i] as JSONObject).get("humidity")}")
            addEntry((dataListJson[i] as JSONObject).get("humidity").toString().toFloat())
        }
    }
    ...
    private fun paintChartData() {
        ...
        descr.text = "Datos de humedad recabados."
    ...
    private fun addEntry(yiData: Float) {
        ...
        if (data != null) {
            ...
            mTempChart.setVisibleXRangeMaximum(20f)
    ...
    private fun createSet(): ILineDataSet? {
        val set: ILineDataSet? = LineDataSet(null, "Humidity data")
        (set as LineDataSet).color = Color.BLUE
